...
const ServicesSection = ({ showMoreButton = true }: { showMoreButton?: boolean }) => {
  const services = [
    {
      id: 1,
  ...
  >
    <Link href={`/services/${service.slug}`} className="block h-full">
      ...
      <div className="bg-[#F8F9FB] h-full rounded-xl p-8 lg:p-10 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-transparent hover:bg-company-deep group cursor-pointer flex flex-col">
        ...
        <ul className="flex flex-col gap-4 mt-auto">
          {service.features.map((feature, idx) => (
            <li
              key={idx}
              className="flex items-center gap-3 text-sm font-semibold text-company-deep group-hover:text-gray-200 transition-colors duration-300"
            >
  ...
  <div className="flex justify-center">
    <ScrollReveal delay={800}>
      <MyButton variant="dark" href="/services">
        More Services
      </MyButton>
    </ScrollReveal>
  </div>
